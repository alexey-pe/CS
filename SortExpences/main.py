...
def main():
    ...
    def amount(element):
        return element[1]
    insertion_sort_desc(expenses, descending, amount)
...
def is_sorted(elements, predicate, keyextractor):
    prev = None
    for cur in elements:
        if prev is not None and predicate(keyextractor(cur), keyextractor(prev)):
            return False
        prev = cur
    return True


def insertion_sort_desc(elements, predicate, keyextractor):
    ...
    assert is_sorted(elements[:k + 1], predicate, keyextractor),\
    ...
    if len(elements) > 0:
        ...
        assert is_sorted(elements[:k + 1], predicate, keyextractor), 'A single element is always sorted.'
        print(elements)

    for k in range(1, len(elements)):
        val = elements[k]
        print('Reached element by index ', k, ' with key ', keyextractor(val), ' and value ', val)
        assert is_sorted(elements[:k], predicate, keyextractor),\
            'Elements up to but excluding the element reached must already be sorted.'
        i = k
        while i > 0 and predicate(keyextractor(val), keyextractor(elements[i - 1])):
